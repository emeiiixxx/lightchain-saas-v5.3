...
- 试衣描述最多2000字，保留展开、保存、提示词库和清空，生成设置默认自动／2K。点击生成要求服装图与非空描述，随后显示未接后端提示，不产生伪造试衣结果；一键融合原有本地示例生成不变。
...
- 锚定模式切换右侧1px并垂直居中；窄屏向下避让，随滚动／缩放窗口更新位置。只在未确认的常规模式显示，点击文案切到内衣；关闭、Escape或选择内衣后以 lc-tryon-lingerie-announcement-dismissed 记住确认。200ms退出后卸载，支持减少动态效果；未运行浏览器检查或测试。
