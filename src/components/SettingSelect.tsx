...
export function SettingSelect({ label, value, icon, options, onChange }: Props) {
  ...
  return <div className="fusion-setting">
    <button ref={trigger} type="button" className="lc-setting-select" data-node-id="66:71" aria-label={label} aria-haspopup="menu" aria-controls={id} aria-expanded={open}
      ...
      <Icon name={icon} size={16} /><span className="lc-setting-select-value">{options.find(option => option.value === value)?.label}</span><Icon name="imgChevron" size={16} className="lc-setting-select-chevron" />
    </button>
