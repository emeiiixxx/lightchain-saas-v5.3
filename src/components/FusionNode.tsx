...
type Props = { image: CanvasImage; locale: Locale; onChange: (patch: Partial<FusionSettings>) => void; onAddMain: () => void; onReference: (source: 'upload' | 'canvas') => void; onDemo: () => void; onNotify: (message: string) => void };
export function FusionNode({ image, locale, onChange, onAddMain, onReference, onDemo, onNotify }: Props) {
  const t = copy[locale];
  ...
  return <section ref={root} className={`fusion-node${image.nodeOnly ? ' is-empty' : ''}`} data-overlay data-node-id={image.nodeOnly ? '107:5118' : '35:6422'} data-source-image={image.id} aria-label={t.title}>
    ...
    <GenerateTaskButton label={t.generate} cost="999" className="fusion-generate" onClick={() => { if (image.nodeOnly) { onAddMain(); return; } if (!settings.prompt.trim()) { setError(true); prompt.current?.focus(); } else onDemo(); }} />
  </section>;
}
